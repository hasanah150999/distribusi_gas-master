...
BEGIN
	...
	IF(data_alat IN(SELECT pel.data_rfid FROM tb_pelanggan pel WHERE pel.data_rfid = data_alat))
		THEN 
			INSERT INTO `db_distribusi`.`tb_log_device` (`data_rfid`, `status`, `jenis_rfid`, `create_date`, `change_date`) 
				VALUES (data_alat, 'N', 'P', CURRENT_TIME(), CURRENT_TIME());
	...
	ELSEIF(data_alat IN(SELECT ag.data_rfid FROM tb_agen ag WHERE ag.data_rfid = data_alat))
		THEN
			INSERT INTO `db_distribusi`.`tb_log_device` (`data_rfid`, `status`, `jenis_rfid`, `create_date`, `change_date`) 
				VALUES (data_alat, 'N', 'A', CURRENT_TIME(), CURRENT_TIME());
			SELECT ag.nama INTO nama FROM tb_agen ag WHERE ag.data_rfid = data_alat LIMIT 1;
			SET message = 'Tapping Kartu: ';
	ELSE
		INSERT INTO `db_distribusi`.`tb_log_device` (`data_rfid`, `status`, `jenis_rfid`, `create_date`, `change_date`) 
			VALUES (data_alat, 'N', 'N', CURRENT_TIME(), CURRENT_TIME());
		SET message = 'Tapping Kartu ';
		SET nama = 'Data Baru';
	END IF;
...
CREATE TABLE IF NOT EXISTS `tb_agen` (
  ...
  `jumlah_pelanggan` bigint(20) DEFAULT NULL,
  `jumlah_tabung` bigint(20) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `tb_pelanggan` (
  ...
  `id_kab_kota` varchar(50) DEFAULT NULL,
